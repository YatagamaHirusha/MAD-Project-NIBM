...
public final class MatchScoring {
    ...
    public static List<MatchSuggestion> computeTopMatches(@NonNull Context context, int limit) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            String photoUrl = demoPhotoUrlForPeerId(s.person.id);
            out.add(
                    new MatchSuggestion(
                            i + 1,
                            s.person.id,
    ...
    /** Stable placeholder image per demo id (Lorem Picsum). */
    @NonNull
    static String demoPhotoUrlForPeerId(@NonNull String peerId) {
        String seed = peerId.replaceAll("[^a-zA-Z0-9]", "");
        if (seed.isEmpty()) {
            seed = "match";
        }
